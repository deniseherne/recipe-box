...
assert.equal(p.meals.filter(m=>m.week===2).length,3);
...
assert.equal(find('eggs','').quantity,16); // 4 for parmesan, 12 frittata
assert.equal(find('heavy cream','tsp').quantity,84); // 4 tbsp + 1 cup + 1/2 cup
assert.equal(find('olive oil','tsp').quantity,57); // mains 15 tsp, sides 42 tsp
assert.equal(find('green beans','oz').quantity,32);
assert.equal(find('bacon slices','').quantity,14);
assert.equal(find('zucchini','').quantity,8);
assert.equal(find('avocados','').quantity,4);
...
assert.match(P.groceryText(p,recipes),/2 lb green beans/);
...
// The standing vegetable exclusion must survive old settings and all optional filters being off.
const unrestricted={keto:false,untried:false,seafood:false,chili:false,beefTips:false,count:14};
for(const vegetable of ['asparagus','cauliflower']) {
  assert.equal(P.eligible({title:'Chicken dinner',ingredients:['2 cups '+vegetable+' rice'],categories:['Chicken']},unrestricted,{}),false);
  assert.equal(P.eligible({title:'Roasted '+vegetable,ingredients:[],categories:[]},unrestricted,{}),false);
}
assert.ok(P.suggest(recipes,P.freshPlan(),unrestricted,()=>({tried:false})).every(r=>!/asparagus|cauliflower/i.test([r.title,...r.ingredients].join(' '))));
assert.ok(!/asparagus|cauliflower/i.test(P.groceryText({...p,meals:P.starterMeals(p,recipes)},recipes)));
const changedRecipes=recipes.map(r=>r.title==='Keto Chicken Parmesan'?{...r,ingredients:[...r.ingredients,'cauliflower crust']}:r);
assert.equal(P.starterMeals(p,changedRecipes).some(m=>m.url.includes('259138')),false);
...
let browser2=browser();assert.equal(browser2.activePlan().meals.length,6);assert.equal(browser2.activePlan().extra,'2 avocados');
reject=true;assert.equal(browser2.changePlanner((s,p)=>{p.meals=[];}),false);assert.equal(browser2.activePlan().meals.length,6);
reject=false;browser2.changePlanner((s,p)=>{const next=P.freshPlan();next.id='another-plan';s.plans.push(next);s.active=next.id;});
assert.equal(browser2.activePlan().meals.length,0);assert.equal(browser2.readPlanner().plans[0].meals.length,6);
